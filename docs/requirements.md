...
| `CAD2SIM-REQ-012` | `MeshQualityEvaluator` | `include/cad2sim/mesh/mesh_quality.hpp`, `src/mesh/mesh_quality.cpp` | `tests/unit/mesh/test_mesh_quality.cpp`, `tests/integration/mesh/test_mesh_quality.cpp` | Surface-triangle mesh quality evaluation covering element count, area, edge lengths, aspect ratio, minimum/maximum angles, degenerate-element detection, and deterministic invalid-connectivity failure handling; unit and integration contracts passed | `VERIFIED` |
| `CAD2SIM-REQ-013` | `RegionIdentifier` | `include/cad2sim/engineering/region_identification.hpp`, `src/engineering/region_identification.cpp`, `include/cad2sim/kernel/validated_shape.hpp` | `tests/integration/engineering/test_region_identification.cpp` | Controlled STEP model produces face-based engineering regions with deterministic references to originating face positions within the validated model representation; repeated identification produces identical region/reference results; invalid validated-shape input fails deterministically; focused integration test passed; full CTest: 14/14 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-REQ-014` | `EngineeringModel` + `EngineeringModelValidator` + `EngineeringModelSerializer` | `include/cad2sim/engineering/engineering_model.hpp`, `include/cad2sim/engineering/engineering_model_serializer.hpp`, `src/engineering/engineering_model.cpp`, `src/engineering/engineering_model_serializer.cpp` | `tests/unit/engineering/test_engineering_model.cpp`, `tests/unit/engineering/test_engineering_model_serializer.cpp` | Structured engineering model validation covers regions, material assignments, analysis entities, boundary-condition-ready references, metadata and region-reference integrity; versioned project-defined serialization is deterministic; deserialization rejects malformed input, unsupported versions, missing required records and invalid references; serialize/deserialize/serialize round-trip preserves the serialized representation exactly; focused unit tests passed; full CTest: 16/16 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-REQ-015` | Deterministic preprocessing regression | `tests/regression/determinism/test_preprocessing_determinism.cpp`, `CMakeLists.txt` | `tests/regression/determinism/test_preprocessing_determinism.cpp` | Repeated execution of the preprocessing pipeline on identical STEP input and explicit meshing parameters produces identical validation results and diagnostics, surface-mesh structure and numerical data, mesh-quality results, engineering-region references, engineering-model validation results, and serialized engineering-model output; focused regression test passed; full CTest: 17/17 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-REQ-016` | `GeometryKernel` + `SurfaceMesher` + `EngineeringModelValidator` | `src/kernel/geometry_kernel.cpp`, `src/kernel/detail/step_shape_loader.cpp`, `src/mesh/surface_mesher.cpp`, `src/engineering/engineering_model.cpp` | `tests/integration/negative/test_error_handling.cpp` | Negative coverage verifies missing STEP input, malformed STEP input, invalid B-Rep geometry, invalid linear/angular meshing parameters, invalid validated-shape input, and invalid engineering-model references; explicit diagnostic assertions are used for stable project-defined diagnostics and malformed STEP requires failure with a non-empty diagnostic; focused test passed and full CTest: 18/18 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-REQ-017` | CMake/CTest automated test suite | `CMakeLists.txt`, `tests/` | Entire registered CTest suite covering core geometry, topology, preprocessing, mesh quality, negative/error handling, and deterministic regression; current baseline: 18/18 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-REQ-018` | CMake/CTest regression suite | `tests/regression/`, `tests/integration/`, `tests/fixtures/step/`, `CMakeLists.txt` | Representative CAD and preprocessing coverage includes simple primitives, topology, feature-recognition cases, invalid/problematic geometry, meshing, engineering-region/model generation, and deterministic end-to-end preprocessing; current full CTest baseline: 18/18 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-REQ-019` | `cad2sim_preprocessing_benchmark` | `benchmarks/preprocessing/benchmark_preprocessing.cpp`, `CMakeLists.txt`, `tests/fixtures/step/` | Repeatable preprocessing benchmark over five representative STEP fixtures with 1 warm-up run and 5 measured runs per case; measures STEP import/validation, surface meshing, mesh quality, region identification, engineering-model validation, serialization and total preprocessing; records topology, mesh, quality and engineering metrics plus process-level peak RSS where measurable; emits human-readable and JSON reports | `VERIFIED` |
| `CAD2SIM-REQ-020` | Documentation Review | `docs/requirements.md`, `docs/architecture.md`, `docs/geometry_model.md`, `docs/meshing.md`, `docs/verification.md`, `README.md` | Documentation review confirms all required documentation areas are present; requirements-to-evidence traceability is documented; implemented capabilities are distinguished from planned/future capabilities; known limitations and explicit project boundaries are documented; focused REQ-020 documentation audit passed | `VERIFIED` |
...
| `CAD2SIM-NFR-002` | `cad2sim_math`, math/geometry unit-test boundary | `src/vec3.cpp`, `src/transform3.cpp`, `CMakeLists.txt`, `tests/unit/math/`, `tests/unit/geometry/` | `cad2sim_vec3_unit`, `cad2sim_transform3_unit`, `cad2sim_primitives_unit`, `cad2sim_advanced_primitives_unit` | Kernel-independent math and geometry components are separately buildable through `cad2sim_math`; the four focused unit-test executables have no OCCT runtime dependency; focused tests passed; full CTest: 18/18 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-NFR-003` | CMake build configuration | `CMakeLists.txt` | `cad2sim_core`, full CTest | Core CMake configuration uses standard CMake/OpenCASCADE discovery without OS-specific conditionals or compiler/linker assumptions; core target builds successfully on the development environment; full CTest: 18/18 passed, 0 failed; the Linux-specific `/usr/include/opencascade` path is confined to the uncommitted REQ-019 benchmark target and is outside the committed core build boundary | `VERIFIED` |
| `CAD2SIM-NFR-004` | README.md, CMakeLists.txt | `README.md`, `CMakeLists.txt` | Clean configure/build + full CTest | README documents CMake 3.20+, a C++17-compatible compiler, OpenCASCADE Technology, and repeatable configure/build/test commands; CMake requires CMake 3.20 and C++17 and discovers OpenCASCADE through `find_package(OpenCASCADE REQUIRED)`; clean build succeeded; full CTest: 18/18 passed, 0 failed | `VERIFIED` |
| `CAD2SIM-NFR-005` | Negative/error handling boundary | `src/kernel/geometry_kernel.cpp`, `src/mesh/surface_mesher.cpp`, `src/engineering/engineering_model.cpp`, `tests/integration/negative/` | `cad2sim_negative_error_handling_integration` | Negative-path integration tests cover missing and malformed STEP input, invalid B-Rep geometry, invalid meshing parameters, invalid validated shape, and invalid engineering-model data; failures are asserted through explicit failure status plus actionable diagnostic messages; focused negative test passed and CTest reports 1/1 passed | `VERIFIED` |
